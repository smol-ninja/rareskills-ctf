...
contract ReadOnlyPool is ReentrancyGuard, ERC20("LPToken", "LPT") {
    ...
    function removeLiquidity() external nonReentrant {
        ...
        uint256 ethToReturn = (address(this).balance * numLPTokens) / totalLPTokens;
...
contract Attacker {
    ...
    constructor(ReadOnlyPool pool_, VulnerableDeFiContract vulnerableContract_) {
        pool = pool_;
        vulnerableContract = vulnerableContract_;
    }
    /**
     * since `_burn` is called after ether transfer, we can manipulate virtual price by taking
     * snapshot before the execution of `removeLiquidity` is finished.
     * At that point, ether has been transferred out of the pool but totalSupply() value remains unchanged
     * 
     */
