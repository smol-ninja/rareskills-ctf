...
/**
 * The only statement that can help us disable flash loan is if somehow we can make the following check fail
 *                      `require(convertToShares(totalSupply) != balanceBefore)`
 * where `balanceBefore` = asset balance of Vault and
 * `convertToShares(totalSupply)` = totalSupply * totalSupply / totalAssets.
 * 
 * Even though the deposit function updates `totalSupply` proportional to `totalAssets`, the contract fails
 * to update `totalSupply` if a user directly transfers token to it.
 * 
 * So just transfer 1 token to the vault and it will revert on flash loan
 * 
 * Check test file for solution
 */