...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Execution', async function () {
        ...
        let exploitContract = await (await ethers.getContractFactory('ExploitContract', player)) .deploy(
            lendingPool.address,
            uniswapRouter.address,
            weth.address,
            token.address
        );
        await token.connect(player).transfer(exploitContract.address, PLAYER_INITIAL_TOKEN_BALANCE);

        expect(
            await token.balanceOf(player.address)
        ).to.be.eq(0);
        expect(
            await token.balanceOf(exploitContract.address)
        ).to.be.eq(PLAYER_INITIAL_TOKEN_BALANCE);

        await exploitContract.connect(player).exploit(
            { value: PLAYER_INITIAL_ETH_BALANCE  - 10n ** 17n}
        );
    });
